...
const KnowledgeBase = ({ onBack }) => {
    ...
    return (
        <div className="h-full flex flex-col bg-white">
            {/* Header */}
            <div className="h-[60px] px-4 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    <button
                        onClick={onBack}
                        className="p-1.5 -ml-2 text-slate-600 hover:bg-white rounded-lg transition-colors"
                    >
                        <ArrowLeftIcon className="w-5 h-5" />
                    </button>
                    <div>
                        <h3 className="font-semibold text-sm text-white">Knowledge Base</h3>
                        <p className="text-[10px] text-slate-600">{files.filter(f => f.active).length} of {files.length} sources active</p>
                    </div>
                </div>
                <button className="text-xs font-medium text-[#37CFFF] hover:text-[#1D57D8] bg-[#1D57D8]/10 px-3 py-1.5 rounded-lg transition-colors">
                    Manage
                </button>
            </div>

            {/* Stats */}
            <div className="grid grid-cols-4 gap-2 p-4 bg-white border-b border-slate-200">
                {[
                    ...
                    { label: 'Active', value: files.filter(f => f.active).length, color: 'text-[#37CFFF]' },
                    { label: 'Docs', value: files.length, color: 'text-purple-600' }
                ].map((stat, i) => (
                    <div key={i} className="text-center p-2 bg-white rounded-xl">
            ...
            <div className="p-4 space-y-3">
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#37CFFF]/20 focus:border-[#37CFFF]"
                    />
                </div>

                <div className="flex items-center gap-2 px-1">
                    <input type="checkbox" className="rounded border-slate-200 text-[#37CFFF] focus:ring-[#37CFFF]" />
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3">
                {loading ? (
                    <div className="flex justify-center py-8">
                        <div className="w-6 h-6 border-2 border-[#37CFFF] border-t-transparent rounded-full animate-spin"></div>
                    </div>
                    ...
                    <div key={file.id} className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm hover:border-[#37CFFF]/20 transition-colors group relative">
                        <div className="flex items-start justify-between mb-2">
                            <div className="flex items-center gap-3">
                                <input
                                    ...
                                    className="rounded border-slate-200 text-[#37CFFF] focus:ring-[#37CFFF] mt-0.5"
                                />
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center bg-white text-slate-600`}>
                                    <DocumentIcon className="w-5 h-5" />
                                </div>
                                <div className="min-w-0">
                                    <div className="text-sm font-medium text-white line-clamp-1">{file.name}</div>
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="p-1 text-slate-500 hover:text-red-500 rounded opacity-0 group-hover:opacity-100 transition-opacity"
                                    title="Delete"
                                >
                                ...
                                <button
                                    ...
                                    className={`w-10 h-5 rounded-full relative transition-colors ${file.active ? 'bg-[#37CFFF]' : 'bg-[#2b303d]'
                                        }`}
                                >
