...
export default function TaskManagement() {
    ...
    const getStatusBadge = (status) => {
        const styles = {
            ...
            'Completed': 'bg-[#2b303d] text-white',
            'Pending Review': 'bg-yellow-500 text-white',
            'Draft': 'bg-[#a2a5ad] text-slate-600',
            'Rejected': 'bg-red-600 text-white'
        };
        return styles[status] || 'bg-white0 text-white';
    };
            ...
            <div className="bg-white px-8 py-6 border-b border-slate-200">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg hover:bg-white transition-colors text-sm font-medium"
                    >
                        Create Task
                    </button>
                </div>
            ...
            <div className="bg-white px-8 py-4 border-b border-slate-200">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 relative">
                        <MagnifyingGlassIcon className="w-5 h-5 text-slate-500 absolute left-3 top-1/2 transform -translate-y-1/2" />
                        <input
                            type="text"
                            placeholder="Search Task Name or Creator"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-10 pr-4 py-2 border border-[#a2a5ad] rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                        />
                    </div>
                    ...
                    <select
                        ...
                        className="px-4 py-2 border border-[#a2a5ad] rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm min-w-[180px]"
                    >
                            ...
                            <tr key={task.id} className="hover:bg-white transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <button
                                        ...
                                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${task.isOpen ? 'bg-slate-900' : 'bg-[#a2a5ad]'
                                            }`}
                                    >
                                        <span
